...
% for j=1:num_of_joints
%     subplot(4,2,j);
%     plot(t(1:10001), ( q_des(j,1:length(t))'  -   results_computed_torque(1:10001,j) )  )
%     legend ()
%     xlabel('time [s]');
%     ylabeltext = sprintf('_%i [rad]',j);
%     ylabel(['Joint position' ylabeltext]);    
%     legend ('Errore Computed Torque')
%     grid;
% end
% 
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
%% Trajectory Tracking: Computed Torque Method disturbance

% Gain circumference parameters matrix
Kp = 0.1*diag([0 1 3 3 5  ]);
Kv = 0.2*diag([48 18 27 20 20 ]);

% Good Helix parameters matrix

q0 = [-0.1047    1.5184   -1.7279    0.1571   -0.2618];
results_computed_torque = q0(1:5)';
index = 1;
q=q0(1:5)' ;
dq=q_dot0(1:5)';
ddq=[0 0 0 0 0 ]';
for i=1:length(t)
   ...
   % Error and derivate of the error   
    err = q_des(1:5,i) - q;
    derr = dq_des(1:5,i) - dq;
    
    %Get dynamic matrices
   
       M_ = PANDAmodel.inertia(q'); 
        C_ = PANDAmodel.coriolis(q',dq'); 
        G_ = PANDAmodel.gravload(q'); 

    % Computed Torque Controller
    
    tau = ( M_*(ddq_des(1:5,1) + Kv*derr + Kp*err ) + C_*dq + G_' );
      
       M = PANDAsymp.inertia(q'); 
        C = PANDAsymp.coriolis(q',dq'); 
        G = PANDAsymp.gravload(q'); 
...
for j=1:n
    subplot(4,2,j);
    plot(t(1:10001),results_computed_torque(j,1:10001))
    legend ()
    hold on
    plot (t,q_des(j,1:length(t)))
    xlabel('time [s]');
...
for j=1:n
    subplot(4,2,j);
    plot(t(1:10001),(q_des(j,1:length(t)) - results_computed_torque(j,1:10001)))
    legend ()
    xlabel('time [s]');
    ylabeltext = sprintf('_%i [rad]',j);
    ylabel(['Joint position' ylabeltext]);    
...
% for j=1:num_of_joints
%     subplot(4,2,j);
%     plot(t,(q_des(j,1:length(t))'-results_backstepping(:,j)))
% 
%     xlabel('time [s]');
%     ylabeltext = sprintf('_%i [rad]',j);
%     ylabel(['Joint position' ylabeltext]);
%     grid;
%     legend ('Errore Backstepping')
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  %% Trajectory tracking: Backstepping control disturbo
% Numero di giunti
n=5;
...
%Good Circumference parameters
Kp = 0.14* diag([1 1 1 1 3 ]);

results_backstepping = q0(1:5);
index = 1;
q=q0(1:5);
dq=q_dot0(1:5);
ddq=[0 0 0 0 0 ];
...
for i=1:length(t)
   ...
   % Error and derivate of the error   
    err = transpose(q_des(1:5,i)) - q;
    derr = transpose(dq_des(1:5,i)) - dq;
    
    dqr = transpose(dq_des(1:5,i)) + err*(Kp);
    ddqr = transpose(ddq_des(1:5,i)) + derr*(Kp);
    s = derr + err*(Kp');
    ...
    % Backstepping Controller
    tau = (M_*(ddqr') + C_*(dqr') + G_' + Kp*(s') + err')';      
       M = PANDAsymp.inertia(q); 
        C = PANDAsymp.coriolis(q,dq); 
        G = PANDAsymp.gravload(q);
...
%             Kp = 0.5*diag([200 200 200 20 10]);
%             Kv = 0.06*diag([200 200 200 200 200]); 
%     % Pesi Backstepping
...
%             Mtilde = PANDAmodel.inertia(q(i-1,:)); 
%             Ctilde = PANDAmodel.coriolis(q(i-1,:),q_dot(i-1,:)); 
%             Gtilde = PANDAmodel.gravload(q(i-1,:)); 
%         % Determino la coppia in base alla scelta
%             switch sel2
%                 case 1  %   Computed torque
%                     tau(i,:) = (e_dot*Kv' + e*Kp' + qd_ddot(i-1,:))*Mtilde' + q_dot(i-1,:)*Ctilde' + Gtilde ; 
%                 case 2  %   Backstepping
...
% for j=1:5
%     subplot(4,2,j);
%     plot(t(1:10001),(q_des(j,1:length(t))'-q(1:10001,j)))
% %     legend ()
%     hold on
%  
%     legend ('Backstepping')
%         xlabel('time [s]');
%     ylabeltext = sprintf('_%i [rad]',j);
%     ylabel(['Joint position error' ylabeltext]);  
%     grid;
% end
% 
